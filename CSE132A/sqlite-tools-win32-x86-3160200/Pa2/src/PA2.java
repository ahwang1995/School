...
public class PA2 {
 ...
 int numStops = 0;
 int rsetSize = rset.getInt(1);
 stmt.executeUpdate("INSERT INTO T SELECT *," + numStops + " FROM Flight;");
 stmt.executeUpdate("DELETE FROM T WHERE T.Stops IS NULL;");
 numStops = 1;
 //System.out.print(rsetSize);
 while (rsetSize > 0) {
	 stmt.executeUpdate("DELETE FROM tOld;");
	 stmt.executeUpdate("INSERT INTO tOld SELECT * FROM T;");
	 stmt.executeUpdate("INSERT INTO T SELECT a.Airline,a.Origin,b.Destination," + numStops +
	 ...
	 numStops++;
	 rset = stmt.executeQuery("SELECT COUNT(*) from D;");
	 rsetSize = rset.getInt(1);
	 System.out.print(rsetSize);
	 System.out.print("\n");
 }
