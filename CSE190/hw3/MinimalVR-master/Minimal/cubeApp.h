	...
	void update() override {
		...
		if (OVR_SUCCESS(ovr_GetInputState(_session, ovrControllerType_Touch, &inputState)))
		{
			...
			if ((inputState.Thumbstick[ovrHand_Right].x) > 0.5f) {
				eye_offset[0] -= deltaTime * vary_speed;
				eye_offset[1] += deltaTime * vary_speed;
			}
			if ((inputState.Thumbstick[ovrHand_Right].x) < -0.5f) {
				eye_offset[0] += deltaTime * vary_speed;
				eye_offset[1] -= deltaTime * vary_speed;
			...
			if (inputState.Buttons > 0) {
				...
				if (inputState.Buttons == 2 && prev_button_state != 2) {
					tracking_state = (tracking_state + 1) % 4;
				}

				if (inputState.Buttons == 4 && prev_button_state != 4) {
					eye_offset[0] = eye_offset_default[0];
					eye_offset[1] = eye_offset_default[1];
				}
