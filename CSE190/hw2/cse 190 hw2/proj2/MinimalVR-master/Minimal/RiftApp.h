...
public:
	...
	float eye_offset_default, eye_offset;
	...
	RiftApp() {
		...
		ovr::for_each_eye([&](ovrEyeType eye) {
			...
			eye_offset_default = erd.HmdToEyeOffset.x; //TEST DIS
...
protected:
	...
	void draw() final override {
		...
		ovr::for_each_eye([&](ovrEyeType eye) {
			...
			if (render_state == STEREO) renderScene(_eyeProjections[eye], ovr::toGlm(eyePoses[eye]));
			else if (render_state == MONO) renderScene(_eyeProjections[eye], ovr::toGlm(eyePoses[ovrEye_Left]));
			else if (render_state == LEFT_ONLY && eye == ovrEye_Left) renderScene(_eyeProjections[eye], ovr::toGlm(eyePoses[eye]));
			else if (render_state == RIGHT_ONLY && eye == ovrEye_Right) renderScene(_eyeProjections[eye], ovr::toGlm(eyePoses[eye]));

			// cse190: this is for normal stereo rendering
